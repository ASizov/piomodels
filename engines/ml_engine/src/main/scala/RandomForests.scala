...
class RandomForestAlgorithm(val ap: RandomForestAlgorithmParams)
  ...
  override def train(sc: SparkContext, data: PreparedData): RandomForestModel = {
    ...
    RandomForest.trainRegressor(labeledPoints, categoricalFeaturesInfo, ap.numTrees,ap.featureSubsetStrategy, ap.impurity, ap.maxDepth, ap.maxBins)
  }
  ...
  override def predict(model: RandomForestModel, query: Query): PredictedResult = {
    println("1888")
    //val features = Vectors.dense(dateStringToMillis(query.fecha), query.agencia_id, query.canal_id, query.producto_id,query.Month,query.Week,query.isholiday,query.isholidaytomorrow,query.popularity,query.isholidayyesterday,query.query.lag1month,query.lag2month,query.lag4month,query.lag05month,query.lag3month,query.fechaLab)
